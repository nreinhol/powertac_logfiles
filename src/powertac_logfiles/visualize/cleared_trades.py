...
def plot_cleared_trades(df_cleared_trades_transformed, time_deltas, game_suffix):
    ...
    plt.title('Cleared Trades', fontsize=visualize.FIGURE_TITLE_FONT_SIZE)

    ax1 = fig.add_subplot(211)
    sns.boxplot(ax=ax1, x="time_delta", y="mkt_price", data=df_cleared_trades_transformed, order=time_deltas)

    ax2 = fig.add_subplot(212)
    sns.boxplot(ax=ax2, x="time_delta", y="mkt_qty_MWh", data=df_cleared_trades_transformed, order=time_deltas)
    ...
    ax1 = fig.add_subplot(211)
    sns.boxplot(ax=ax1, x="hour_of_day", y="mkt_price", data=df_cleared_trades_transformed, order=[i for i in range(0, 24)])

    ax2 = fig.add_subplot(212)
    sns.boxplot(ax=ax2, x="hour_of_day", y="mkt_qty_MWh", data=df_cleared_trades_transformed, order=[i for i in range(0, 24)])
